...
class CBORDecoderTests: XCTestCase {
	...
	func testDecodeNumbers() {
		...
		do { let _ = try CBORDecoder(input: [0x19, 0xff]).decodeItem(); XCTAssertTrue(false) } catch { XCTAssertTrue(true) }
		...
		do { let _ = try CBORDecoder(input: [0x1a]).decodeItem(); XCTAssertTrue(false) } catch { XCTAssertTrue(true) }
        ...
        do { let _ = try CBORDecoder(input: [0x1b, 0x00, 0x00]).decodeItem(); XCTAssertTrue(false) } catch { XCTAssertTrue(true) }
	...
	func testDecodePerformance() {
		...
		data.append(0xff)
		self.measure {
			let _ = try! CBORDecoder(input: data).decodeItem()
		}
	}
